...
import { canonicalSha256, createComplianceRequest, validateComplianceResponse } from "../../apps/browser-workbench/src/compliance-client.js";
...
async function responseFixture() {
  ...
  const observation = await stamp({ schema_version: "caddydaddy.compliance-observation/1", observed_at: "2026-09-05T20:00:00Z", producer: { system: "TRIPWIRE", runtime: "SERVER", source_commit: "8".repeat(40) }, evaluation_state: "SUCCEEDED", input_ref: inputRef, forge_revision_ref: { revision_id: request.forge_revision_id, content_hash: "2".repeat(64) }, rule_pack: { state: "DRAFT_REVIEW_ONLY", content_hash: "3".repeat(64) }, findings: [{ finding_id: "finding:one", node_id: "tripwire-node:one", outcome: "INSUFFICIENT_EVIDENCE", rule_ids: [], reason_codes: ["DRAFT_REVIEW_ONLY"] }], review_requirement: "HUMAN_REVIEW_REQUIRED", claim_ceiling: "REVIEW_SUPPORT_ONLY_NO_LEGAL_CONCLUSION", legal_effect: "NONE" }, "observation_id", "observation_hash", "compliance-observation:");
