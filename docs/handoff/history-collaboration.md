...
12. Emit generic evidence-linked observations under `docs/contracts/now-observation.md` so Shipyard can show actual lane state, exact holds/unblocks, checks, and candidates without copying prompts or private paths. This is an observation adapter, not a second progress ledger.
