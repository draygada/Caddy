...
| `POST /api/sourcing/rounds/{id}/{verb}` | one verb per call: `resolve screen rescreen cost refine select adjudicate resolve_escalation gate declare package propose accept_proposal reject_proposal` | a lane refusal is **409** with the candidate envelope beside the refusal's words; a malformed or incomplete body is **422** |
...
8. **Charlie's rulings, 2026-09-06 ("apply all six as recommended") plus two residuals — commit `6e51feace56e8e55a8eb5a7d545f97c22459033f`.** Every item has a covering test; 173 passed, every earlier test green.
