...
- Branch: **`lane/sourcing-live-cache`**, created on Charlie's instruction off `main @ 3e834f1afa78366963347351515ec4d2def91024` (the merge of PR #3, `lane/sourcing-rulings`; PR #2 before it merged `lane/sourcing-search`, itself cut off `lane/sourcing` @ `890f955` on `main @ 92241b9ca7c9df37cc48e55b4ea388cb21686bb4`). Candidate commit `876b021` (the eval harness reads the best card per candidate mpn), on top of `cc0dad1` (the structured-output schema fix that made the live run possible); the recorded cache and the corrected gold row (`8326a9b`) and this paperwork commit are its successors and change no runtime source.
- Worktree label `forge/sourcing`; writable `packages/sourcing/**`, `tests/sourcing/**`, `governance/receipts/sourcing.json` — all 26 changed paths since `3e834f1` are inside that allowlist (custody output at the foot of this file; the check's base is this branch's merge-base with `main`, because `main` also carries PR #1's frontend, kernel and service paths, which are not this lane's).
- Registry: **not admitted**. The lane is proposed in `packages/sourcing/handoff/custody-lane-entry.proposed.json`; its write allowlist, worktree and worktree label are unchanged from the original proposal, and only the `branch` field is kept at the branch that exists (`lane/sourcing-live-cache` since this PR) so the check and the integrator both read it. No shared file was edited from the lane: `governance/custody.v1.json` and `tools/check_custody.py` are untouched, and the custody run below used a patched copy of the tool over a merged registry in the coordinator's gitignored scratch directory.
- Authority: Charlie Haywood's in-session instructions, 2026-09-05/06 ("implement the sourcing back end", then the part-search lane on a separate branch, then record the model cache from one live run and land it). **Not pushed**; merge to `main` is not requested here.
...
| `record_cache.py` | `record-cache` | **LIVE** model calls under the spend cap; requires `ANTHROPIC_API_KEY`; the only script that spends money. Run once, 2026-09-06; `data/llm_cache/` is committed, so this is a **re-recording** and costs another run |
...
It now carries **12** rows: the five `fixture://` documents above and the seven real vendor documents the seeding fetched (the pool names eight real urls) (status `200`, sha256, byte count and retrieval date each). The eighth real url in the pool, `https://www.st.com/resource/en/datasheet/stm32f100c8.pdf`, is absent because the host timed out for a plain client; `https://estore.st.com/en/stm32f100c8t6b-cpn.html` answered. Document bytes stay out of the repository; only their hashes are committed.

### The recorded model cache (one live run, 2026-09-06)

`data/llm_cache/` holds **13** recorded prompts, one file per `sha256(kind + "\n" + prompt)`, each `{kind, model, prompt_sha256, response, usage}`. Charlie ran `make record-cache` once with his key, after `cc0dad1` fixed the structured-output schema (every earlier live call had abstained on `provider error: BadRequestError` — the API rejects `maxItems` / `minLength` / `minimum` / `pattern`).

| Measured on that run | Value |
|---|---|
| Provider calls | **23** live calls for **13** unique prompts — the recorder re-calls on a repeated prompt and the last response wins, so ten calls bought nothing (a next step below, not a defect) |
| Budget | about **$1.97** reserved by the pre-call estimate; priced from the recorded `usage`, the 13 cached calls cost **$0.39** |
| Largest single call | the ICM-42688-P full-datasheet extraction: 140,610 input tokens on `claude-sonnet-5`, **$0.29** |
| Replay | `make eval` and `scripts/eval_search.py --mode cache --assert-cached` both exit 0 with **`cache_misses 0`** |

`make eval` over `data/search/gold_swaps.json`, replayed from that cache (six rows; two of them, IMU-NG and GX-220, are synthetic by construction, so this is a per-row record and never a rate):

| Row | Real? | Expected | Observed | Pass |
|---|---|---|---|---|
| `thermal_lepton_after_boson` | REAL | green | green | ✓ |
| `imu_icm_after_hg5700` | REAL | grey | grey | ✓ |
| `imu_ng_synthetic` | SYNTHETIC | red | red | ✓ |
| `gx220_poisoned` | SYNTHETIC | red | red | ✓ |
| `cell_p45b_after_sicore` | REAL | grey | grey | ✓ |
| `io_mcu_origin_escalation` | REAL | grey, not confident | grey, not confident | ✓ |

Alongside them: verifier acceptance **22 accepted / 10 rejected**, every rejection `span_not_found`; `false_green_structural 0`; `abstain_reasons {}`; `cost_per_green_microusd 35264` (millionths of a dollar per passing green row; one row qualifies, at $0.035). Those two acceptance numbers assume `make seed` has filled `.cache/fetch/`; the six pass/fail rows do not (see the `.cache/` note above).

**What the 10 `span_not_found` rejections are.** Reproduced offline from the committed cache, span by span: in **every one of them the quoted sentence is present in the document verbatim** — `text.find(quote)` returns a real index, never −1. What fails is the **character offset the model reported**, and the verifier binds a figure only when `text[start:end] == quote`. Five distinct (prompt, spec) rejections make ten: the two on the ICM datasheet are counted three times, because the IMU slot's search returns the same four candidates in each of the three IMU rows.

| Document | Field | Model said | Quote actually at | Off by |
|---|---|---|---|---|
| ICM-42688-P datasheet (real PDF) | `gyro_rate_range_deg_s` | 47049–47077 | 47049, 27 chars | end +1 |
| ICM-42688-P datasheet (real PDF) | `accel_linear_range_g` | 49064–49090 | 49064, 25 chars | end +1 |
| Lepton Series datasheet (real PDF) | `resolution_w` / `resolution_h` | 3510–3537 | 3506, 26 chars | start +4 |
| Molicel P45B datasheet (real PDF) | `energy_density_wh_kg` | 1025–1034 | 1024, 9 chars | start +1 |
| `fixture://molicel_p45b_test_excerpt.txt` | `energy_density_wh_kg` | 358–379 | 357, 21 chars | start +1 |

So nine of the ten are on real vendor PDFs and one is on a team-authored fixture sheet; the fixture sheets are not immune, and the split is not real-versus-fixture but long-document-versus-short. This is the verifier doing the job it exists for — a figure whose span cannot be re-found at the offsets given is refused, and no unbound number reached a card — and it is **not** a text-fidelity problem in `pypdf`. The next pass belongs in the prompt or in a bounded re-anchor (re-find the quote near the stated offset and accept only an exact, unique byte match), never in relaxing the equality test.

Two consequences a reader should carry:
- `cell_p45b_after_sicore` binds **no** span in this replay — both its documents lost their only figure to the offset — so its card is grey for "no accepted span" as well as "rate not verified". The gold row still passes (it expects grey and the phrase "rate not verified" is printed), but its `note` — "242 Wh/kg from the v1.4 datasheet … green once Charlie's HTS pass flips 8507.60 to verified" — describes the intent, not this run: after an HTS pass the card would still be grey until a span binds. Left as it stands, flagged here for Charlie rather than edited quietly.
- The real Lepton datasheet bound one figure of three, so the thermal row's green comes from the **fixture** test sheet, not from the vendor PDF. The proposal is green on the candidate because one of its three cards concluded; the real-PDF card is grey and says why.

`.cache/` is gitignored. After `make seed` expect `.cache/fetch/` to hold the fetched vendor documents plus the fetch manifest and the BLOCKED log. Measured on the 2026-09-06 seeding: seven of the eight real urls answered `200`, including `estore.st.com`; `www.st.com`'s datasheet PDF timed out for a plain client, and digikey.com and mouser.com bar automated retrieval and are never requested. Those bytes are what the real-document cards read. Measured on a throwaway fetch directory: **without** `make seed` the six gold rows still pass and `cache_misses` is still 0 — every real url degrades to `OFFLINE` and the fixture cards carry the result — but verifier acceptance reads `21 accepted / 1 rejected` instead of `22 / 10`. So a fresh clone runs `make seed` before quoting any acceptance figure. `make eval` writes `.cache/eval/results.jsonl` and `errors.jsonl`; the suite writes `.cache/last_pytest.json`, which `/now` reads back. `make refresh-csl` writes `.cache/csl/`.

## Tests

`packages/sourcing/.venv/bin/python -m pytest tests/sourcing -q` → **178 passed in 1.05 s** on `876b021` (the eval-harness candidate; 177 on `cc0dad13` with the schema fix's three tests, and 174 on `1ba3aa0bc37b`, the merge-gate fix candidate merged in PR #3, before it).

Families: **S1** (resolve every line; round binds the design state; unknown ownership is a flag not a block; depth tier printed; the motor's 301 line and the §848 amber; the cells' MPF minimum and Taiwan 10 %; domestic "no entry"; claim ceilings on every card; event order; idempotent open) · **S2** (the cheaper motor second and blocked with both lists named; selection records declined reason, status and classification state; reason defaults; refusals for a blocked offer, an unknown reason code, a missing attestor; re-selection appends a successor; round confirms when every line is selected; two adjudication roles bound to the snapshot; refuse an analyst escalation and an unknown role) · **S3** (supersession with the seq in the note; the gate as the engine cell; typed reference lifts the block and is never validated; the three-line declaration rule) · **S4** (re-derive covers both lanes; tamper on the declined reason breaks at that seq) · the three no-change edits · package · order · cost hash · claims vocabulary · boundary — plus **documents** (sniffing, hidden line kept and named, exact offsets, sha over utf-8 bytes, same-process determinism and the recorded pin) · **fetch** (allowlist and the blocked demo hosts, blocked hosts logged and never requested, offline cache, `fixture://` cannot escape the fixtures directory, a missing fixture is an error not an exception, the CSL path passes through, a refreshed list's date is its manifest's or not verified) · **schemas / verifier** (the eight forced outcomes, the unit trap, adjacency, comma refusal, ambiguity by group, dimension tuples, the stored figure is the document's, a number glued to a non-unit letter never binds, no classification slot) · **model port** (scripted order then abstain, cache miss then replay with usage, budget reserve and breach, live model abstains without a key and never touches the network, recording model fills and replays) · **Call A** (accepts the frame rate and records the prompt hash, abstains on a schema-invalid response and on a model abstain, and the **poisoned page**'s three outcomes end at the rule or at REJECT) · **rules / dry-run** (decimal-string thresholds and the sha, fields from the rows that apply, fires / does not fire / cannot conclude, tree rows not evaluated, a true `not` guard suppresses the row and a false or missing one never does, a conjunctive `all` guard suppresses only when every one of its atoms holds where an `any` guard suppresses on any one) · **fit** (all four dimensions, unresolved fields, case-insensitive substrings) · **evaluate on copies** (green, grey on an unpublished field, grey on an unverified rate, red on the USML row and on the tripped row, no-specs grey, ranking, a green card names the tree rows it deferred; the round digest is unchanged) · **proposals** (recorded as agent events, the stored proposal is a copy and candidate urls are in the chain, escalation over the pool and a human acceptance, a source the pool never listed is ignored, budget exhaustion is an abstain not a crash, a budget breach after a read candidate is grey not green, accepted/rejected lifecycle refusals) · **claims over proposals** (the never-say grep over a round carrying proposals) · **refresh parsers** (HTS free/percent, CROSS number-subject-date, GLEIF rows with a url and no percentage) · **eval harness** (the six measurements without a percentage; cache misses counted and abstains bucketed; a candidate carrying two documents is observed on its best card) · **engine adapter** (the Wave-0 response becomes `open_round` input; the 848 row becomes the amber flag; a bad revision is refused; `cannot_evaluate` rows reach `unresolved`; an unresolved rule blocks the declaration and the words name the cause; the canonical check says matched or not_checked, never differs; the root product's determination rides in the part-node shape) · **API and page** (health/states/envelope, `/now` read-only and defaulting to UNKNOWN, a lane refusal is 409 with the shipped body, malformed body 422, missing key 422 naming the key, unknown state 422 naming the states, select and a refusal, propose records an agent event whatever the cache holds, re-derive and tamper, a 409 carries the candidate envelope, and the missing-key, malformed-body and non-integer-quantity 422s carry their shipped bodies, from the app and from a router-only mount alike, a non-integer refine quantity is 422, the page renders from the response and says nothing forbidden) · **docs** (the glossary names the search terms, the ADRs and README carry the decisions and the claim sentence, every emitted kind is in the schema proposal).
...
10. **The API receives a schema stripped to the structured-output subset — commit `cc0dad1376de0333c6d0860c381234720c94c979`.** Every live call abstained on `provider error: BadRequestError` (400 `output_config.format.schema: For 'array' type, property 'maxItems' is not supported`): structured outputs accept only a subset of JSON Schema, and `EXTRACT_SCHEMA` / `SEARCH_SCHEMA` carry `maxItems`, `minLength`, `minimum` and `pattern`. `schemas.api_schema()` returns a deep copy with the unsupported keywords removed recursively and `_complete` sends that. **Nothing is relaxed**: `extract.py` and `propose.py` still run `validate(response, <full schema>)` on the reply, and that is the fail-closed gate for the caps, patterns and minimums. Three covering tests: the live request carries `api_schema(...)` and nothing else changes; the strip removes those keywords everywhere in the tree (property names included, so a stray keyword cannot hide as one) and keeps every `additionalProperties: false`; and each stripped keyword is **still** a violation at the caller, against the full schema the API never saw.
11. **The eval harness observes the best card per candidate mpn — commit `876b021`, with the gold correction in `8326a9b`.** `candidate_pipeline` emits one card per (mpn, url), so a pool candidate with three documents yields three cards. `run_gold` took the **first** card for the expected mpn, so the Lepton row read grey — the real-PDF card, whose `6A003.b.4.b (RS1)` branch cannot fire because the element count is not published — while the proposal on that same candidate was green and confident. A gold row names a candidate, not a document: `observed` is now the best card by `evaluate.STATUS_ORDER` (green < grey < red), green whenever the proposal ranked that mpn, and the phrase check reads every card for that mpn. It stays **card-scoped** on purpose — admitting the proposal's own headline would make a row like `gx220_poisoned`'s `7A002.a.1.a` vacuous, because the headline names the rows the CURRENT part fired. A row with no `expect.mpn` (the escalation) is unchanged. Covering test: a scripted model whose one candidate carries two documents, the unreadable one first, must be observed green. `io_mcu_origin_escalation` separately expected "no source resolved", written for a stricter assumption — that neither st.com host would answer; the estore page did, so the row now looks for "not confident", the phrase every escalation prints.
...
Since 2026-09-06 that list also holds, on **one** recorded live run: real model answers replayed from `data/llm_cache/` through all six gold rows with zero cache misses, and real vendor documents read end to end — fetched under the allowlist, text through the pinned `pypdf`, extraction, verification, dry-run, walk, screen and cost.

**Not shown, not claimed:** any classification, customs or legal outcome (every output carries its ceiling sentence); ownership completeness (the table is what was typed); any tariff rate as correct (base rates are expected values); that a typed authorization reference is valid; any real order, supplier contact or filing; signing — plus **repeatable live model behaviour** (one run, one model pair, one prompt version; nothing about variance, and the demo replays the recording rather than calling out); **reliable figure binding out of real PDFs** (across the nine distinct extraction prompts, the model quoted six figures out of a real vendor PDF and ten out of the fixture sheets; one of the six bound and five were refused on the offsets it gave, against nine of ten on the fixtures); two-machine determinism; that the search finds anything **outside the pool**; and any accuracy figure.
...
- **The recorder re-calls live on a repeated prompt.** 23 provider calls produced 13 unique cached prompts on the 2026-09-06 run: the extra ten paid for answers the cache then overwrote (last response wins). Reading the cache before each call, and recording the two identical IMU-slot searches once, is the cheap next pass — worth doing before any second run.
- **Why five spans were refused on their offsets** (table above): whether the drift is the model's counting on long documents, a prompt that never asks it to re-check, or something in how the text is presented. A bounded re-anchor near the stated offset with an exact, unique byte match is the candidate fix; nobody has written it.
- **One run only.** Nothing is known about variance between runs, between models, or after a `PROMPT_VERSION` bump — a bump invalidates every cache key and needs another paid run.
- **`https://www.st.com/resource/en/datasheet/stm32f100c8.pdf` never resolved** (timeout for a plain client), so the escalation has one source, not two; digikey.com and mouser.com remain expected failures and were never fetched.
...
Nothing on `main` changed. Delete the branch (`git branch -D lane/sourcing-live-cache`; `main @ 3e834f1` already carries PR #3), remove the worktree, and remove `packages/sourcing/.cache`. Dropping the branch drops the recorded model cache with it, and re-recording it costs another live run.

## What the page said (Task 5.3 walk-through)

**Dated record, partly superseded.** This walk-through ran at git `6fd7050`, before the cache was recorded; step 5 is the only step the recording changes, and it is marked below. It is kept as observed and not rewritten — the page has not been walked again since.

Controller's browser walk-through, 2026-09-06, git `6fd7050`, `TRIPWIRE_LLM=cache`, `data/llm_cache` empty at the time. Server: `make serve` (uvicorn 127.0.0.1:8000). Page: `GET /` (200, text/html). Envelope on `/health`: `git_sha 6fd70505…`, `mode {llm: cache, api: cached}`, `pool_sha256 56682ab9…`, `rules_sha256 ac95bcdd…`, fixture shas `offers@fb1a82ef ownership@51960858 tariff@c4e68789 csl@fa863267`.
...
5. "Propose alternative" on thermal_core (no committed cache **at the time**) → Proposals section: `grey · alternative · line:thermal_core CACHED [Reject]` / `abstained: cache miss`. Timeline: `#138 proposal agent proposed 0 candidates for line:thermal_core · grey · not confident · CACHED · abstained: cache miss · pool 56682ab9 · rules ac95bcdd`. **Superseded by the recorded cache:** the same click now replays three cards for `500-0771-01` and the proposal reads `green · confident · 3 candidates read · 1 green · 2 needs input · 0 rejected`, measured through `run_gold` on the same code path, not in the browser — nobody has re-walked the page.
...
Run against the proposed lane row, because the registry does not carry this lane yet: the repository's `tools/check_custody.py` and `governance/custody.v1.json` were **not modified**; a copy of the tool with `REPO_ROOT` and `REGISTRY_PATH` rewritten was run over a merged registry (the repository's registry plus `packages/sourcing/handoff/custody-lane-entry.proposed.json`), both written under the coordinator's gitignored scratch directory. The base is `3e834f1`, this branch's merge-base with `main` (`make custody` reads it from `git merge-base origin/main HEAD`): the lane's original base `92241b9` now sits behind PR #1's frontend, kernel and service paths on `main`, so a diff from it would list paths this lane never wrote. The path list now carries the 13 recorded cache files.

```
lane=sourcing
branch=lane/sourcing-live-cache
base=3e834f1afa78366963347351515ec4d2def91024
head=HEAD
changed_paths=26
  governance/receipts/sourcing.json
  packages/sourcing/HANDOFF.md
  packages/sourcing/README.md
  packages/sourcing/data/llm_cache/01c3d2c3f89ac4a91078308b9f5e8050df126de6b3fa53f70af75bf6f7aa55b8.json
  packages/sourcing/data/llm_cache/03c99a362fe889e656b0ad68caaedaaff000d531e03b58fa5d479bd030893b79.json
  packages/sourcing/data/llm_cache/197938f04ddee0330e3235584b9ddc4ae0ef723ec74d80f2f43c3a3a92e7cb13.json
  packages/sourcing/data/llm_cache/1b0abc6525547bb74b143ea2c2493d27c506bed21b159d6bd0a648d3063d7e76.json
  packages/sourcing/data/llm_cache/441d2c47bb130c82f792b24618008e50b6f59f7de02bbe3eb99dddcf21b65a76.json
  packages/sourcing/data/llm_cache/804e156deabd7660dcb60531c2357ad764b918a70ba04d66ed36916b905e9302.json
  packages/sourcing/data/llm_cache/84ce8ee680067a921556eddaffc972475b8b69faa16d37bb0e420ee1e2b61396.json
  packages/sourcing/data/llm_cache/98fd657411fe0b4facb8de0df8d2fca13a7be3f690ef184350cd34bc6303cf16.json
  packages/sourcing/data/llm_cache/9b5d4c6f81c57843119b5dc170fcbb5e4825d6fff97382d299c353793730a209.json
  packages/sourcing/data/llm_cache/ae7bfa638ff17dbd47ca8793ab14acb73fa03b6ac2facdce932cb3fd2240393d.json
  packages/sourcing/data/llm_cache/e74ee7810d8318e528072711a111ad04a6f3ff85d4d740dd5f7a7324f32b537d.json
  packages/sourcing/data/llm_cache/ec4401c0a49681cf6f22fa15c03f2e3d87eeb50101cd7bd19d31804f18835676.json
  packages/sourcing/data/llm_cache/f6b07e1fc864269ab8d67f95ef2ad04edbcbfd237de0e1c1d24f3361bb92d147.json
