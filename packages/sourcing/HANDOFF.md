...
- Branch: **`lane/sourcing-search`**, created on Charlie's instruction off `lane/sourcing` @ `890f955`; the lane is based on `main @ 92241b9ca7c9df37cc48e55b4ea388cb21686bb4`. Candidate commit `6fd70505b7ca8dbfe89a723b19d843fa9890c295` (Task 6.1); this handoff commit is its direct successor and changes no runtime source.
...
`packages/sourcing/.venv/bin/python -m pytest tests/sourcing -q` → **144 passed in 0.73 s** on `6fd70505b7ca`.
...
8. **The 848 row's `part_class_in` names `flight_controller` / `radio` / `camera` / `gimbal`;** the lane's classes are `thermal_imager` / `radio` / `board`, so the flag row never fires in the dry-run. A mapping gap for the engine, not sourcing logic.
...
6. **Wiring** — `frontend/vite.config.ts` already proxies `/api` to `127.0.0.1:8000` with no rewrite, so mounting this router at `/api/sourcing` needs no frontend change; `features/tripwire/vite.config.js` has no proxy. The FB-04 integration owner mounts `forge_sourcing_api.app.router` into `features/tripwire/backend/app.py` (one `include_router` line) **or** runs `make serve` on 8000 during the demo.
...
- `verify.py` `TUPLE` accepts a letter-glued first component — "Rated IP67 by 2 mm" accepts `67 mm`, "0x20 mm" accepts `0`. Fix: `(?<![A-Za-z])` before the tuple's first number. This is the trust boundary, so it is the first of these to close.
- `service.py` proposal lifecycle guard tests truthiness, so `reject_proposal(attestor="")` stamps an empty attestor and a second reject stacks. Fix: `"accepted_by" in p or "rejected_by" in p`. (Accept is safe — the thread refuses an empty attestor on terminal kinds.)
- `fetch.py` a `fixture://` name containing a NUL byte raises `ValueError` out of `fetch` instead of returning an error result. Unreachable through the verbs; confinement holds.
- `engine_adapter.py` the `unresolved` dedupe is keyed on `rule_id` alone, so the same rule propagating from two children with different cause nodes merges into one row. Fix: key on `(rule_id, entry_id, cause_node_id)`.
- `POST /packets/{unknown}/close` returns 500 because `service.close_order` subscripts `self.packets` bare (dispatch raises `OrderRefused UNKNOWN_PACKET` properly). One-line seam fix.
- `engine_adapter`'s `destinations_note` composes the engine's free text and is outside the never-say test's scan; running `strings(round_view)` over an adapter-built round would close that.
- `LiveAnthropicModel` appends to `self.calls` only on success, so after an abstain `calls[-1].usage` reports the **previous** call's tokens. Live path only, untested, and worth closing before the recorded run.
- The escalation verb's budget catch sets `abstained` but appends no abstain word (the alternative verb does).
