...
(a guard keyed `all` holds only when every one of its atoms does, and names them all); a guard that is false or
whose fact is missing never suppresses (fail-closed).
"""
...
def _guard(guard: list[dict], fields: dict, declared: dict, part_class: str | None) -> tuple[str, str]:
    """A `not` guard holds only when every one of its atoms is true; an atom that is false, unpublished or tree-bound never lets it
    hold (fail-closed), and an empty guard never holds. A conjunctive guard names every atom, joined with " and "."""
    results = [_atom(a, fields, declared, part_class) for a in guard]
    return ("true" if results and all(s == "true" for s, _ in results) else "false"), " and ".join(d for _, d in results)
...
def dry_run(rules: dict, *, part_class: str | None, role: str | None, fields: dict[str, dict], declared: dict) -> dict:
    ...
    for row in rows_for(rules, part_class, role):
        ...
        held = next((d for s, d in (_guard(g, fields, declared, part_class) for g in guards(when)) if s == "true"), None)
