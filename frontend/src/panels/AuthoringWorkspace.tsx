...
export function AuthoringWorkspace({ fetchImpl = fetch, initialDocument }: AuthoringWorkspaceProps) {
  ...
  return (
    <section data-cad-authoring-workspace aria-labelledby="cad-authoring-title" className="cad-authoring-workspace" style={{ height: '100%', minHeight: 0, overflow: 'auto', background: 'var(--bg)', color: 'var(--ink, var(--ink))' }}>
      <header style={{ padding: '12px 14px', borderBottom: '1px solid var(--line)', background: 'var(--surface) 0%, var(--surface2) 55%, var(--surface2) 100%)' }}>
        ...
        <div aria-label="QX-0 hardened drone benchmark" style={{ marginTop: 8, padding: '8px 10px', border: '1px solid var(--line)', borderRadius: 6, background: 'var(--surface2)', display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 7 }}>
          <div style={{ flex: '1 1 260px', minWidth: 0 }}><b style={{ fontSize: 12 }}>QX-0 hardened CAD case</b><div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 2 }}>12 BOM definitions · 25 inert instances · 24 recorded mates · public synthetic geometry only. Generic dimensions and non-fixed mates remain non-solving in browser mode.</div></div>
...
function SketchEditor({ sketch, busy, onChange, onCommit }: { sketch: CadSketch; busy: boolean; onChange: (sketch: CadSketch) => void; onCommit: () => void }) {
  ...
  return <section aria-labelledby="sketch-editor-title" style={{ ...card, padding: 10, display: 'grid', gap: 7 }}>
    ...
    <div style={{ borderTop: '1px solid var(--line)', paddingTop: 6, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 5 }}>
...
function EntityRow({ entity, onChange, onRemove, index }: { entity: SketchEntity; onChange: (entity: SketchEntity) => void; onRemove: () => void; index: number }) {
  ...
  return <div style={{ display: 'grid', gridTemplateColumns: '62px 1fr auto', gap: 4, alignItems: 'center', background: 'var(--surface2)', padding: 5, borderRadius: 6 }}><span style={{ ...mono, fontSize: 12 }}>{index + 1}. {entity.kind}</span><input aria-label={`${entity.kind} geometry values`} value={values.join(', ')} onChange={(event) => onChange(updateEntityNumbers(entity, event.target.value.split(',').map(Number)))} style={{ ...field, padding: '5px 6px', fontSize: 12 }} /><button type="button" aria-label={`Remove ${entity.kind}`} onClick={onRemove} style={{ ...button, padding: '4px 6px' }}>x</button></div>;
}
