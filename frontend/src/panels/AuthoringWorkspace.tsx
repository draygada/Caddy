...
const card: CSSProperties = { border: '1px solid var(--line2)', borderRadius: 6, background: 'var(--surface)' };
const mono: CSSProperties = { fontFamily: 'Geist Mono, ui-monospace, monospace' };
const field: CSSProperties = { width: '100%', minWidth: 0, minHeight: 32, padding: '0 10px', border: '1px solid var(--line)', borderRadius: 6, background: 'var(--surface)', color: 'inherit', font: 'inherit', fontSize: 13 };
const button: CSSProperties = { border: '1px solid var(--line)', borderRadius: 6, background: 'transparent', color: 'inherit', minHeight: 32, minWidth: 32, padding: '0 12px', fontSize: 13, fontWeight: 500, cursor: 'pointer' };
const actionButton: CSSProperties = { ...button, background: 'var(--accent)', borderColor: 'var(--accent)', color: 'var(--accentfg)', fontWeight: 600 };
...
export function productOutputRegistration(bundle: CadOutputBundle, acceptedCad: ProductCadRevision): RegisterProductOutputsInput {
  ...
  const isKnownBomPath = (path: string) => path === 'bom.csv' || path === 'bom/bom.csv';
  const bomArtifacts = bundle.artifacts.filter((artifact) => isKnownBomPath(artifact.path));
  const packageBomDescriptors = bundle.package.artifacts.filter((artifact) => isKnownBomPath(artifact.path));
  const bomArtifact = bomArtifacts.length === 1 ? bomArtifacts[0] : undefined;
...
export function AuthoringWorkspace({ fetchImpl = fetch, initialDocument }: AuthoringWorkspaceProps) {
  ...
  const statusColor = state.status === 'failed' || state.status === 'stale' ? 'var(--red)' : state.status === 'running' || state.status === 'queued' ? 'var(--amber)' : 'var(--accent)';
  ...
  return (
    <section data-cad-authoring-workspace aria-labelledby="cad-authoring-title" className="cad-authoring-workspace" style={{ height: '100%', minHeight: 0, overflow: 'auto', background: 'var(--bg)', color: 'var(--ink)' }}>
      <header style={{ padding: '12px 14px', borderBottom: '1px solid var(--line)', background: 'var(--surface2)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', flexWrap: 'wrap', gap: 12 }}>
          <div>
            <div style={{ ...mono, fontSize: 12, color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: '.1em', fontWeight: 850 }}>{engineLabel}</div>
            <h2 id="cad-authoring-title" style={{ margin: '4px 0 2px', fontSize: 23 }}>CAD authoring workshop</h2>
            <div style={{ fontSize: 12, color: 'var(--muted)' }}>{state.document.name} · draft {shortId(state.document.revisionId)} · rendered {shortId(state.lastValidDocument.revisionId)}</div>
          </div>
          <div role="status" aria-live="polite" style={{ ...mono, padding: '7px 9px', border: `1px solid ${statusColor}`, borderRadius: 6, color: statusColor, background: 'var(--surface)', fontSize: 12, fontWeight: 850, textTransform: 'uppercase' }}>{state.status} · {state.kernel ? `${state.kernel.name} ${state.kernel.version}` : 'kernel not yet proven'}</div>
        </div>
        <div role="note" style={{ marginTop: 9, padding: '8px 10px', borderLeft: '4px solid var(--amber)', background: 'color-mix(in srgb, var(--amber) 12%, var(--surface))', fontSize: 12, lineHeight: 1.4 }}>
        ...
        <div aria-label="QX-0 hardened drone benchmark" style={{ marginTop: 8, padding: '8px 10px', border: '1px solid var(--line)', borderRadius: 6, background: 'var(--surface2)', display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 7 }}>
          <div style={{ flex: '1 1 260px', minWidth: 0 }}><b style={{ fontSize: 12 }}>QX-0 hardened CAD case</b><div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 2 }}>12 BOM definitions · 25 inert instances · 24 recorded mates · public synthetic geometry only. Generic dimensions and all mates remain recorded, non-solving intent in browser mode.</div></div>
      ...
      {(state.error || formError) && <div role="alert" style={{ margin: '10px 12px 0', padding: 9, background: 'color-mix(in srgb, var(--red) 10%, var(--surface))', border: '1px solid var(--red)', borderRadius: 6, color: 'var(--red)', fontSize: 12 }}>{formError ?? state.error} <button type="button" onClick={() => dispatch({ type: 'recover-last-valid' })} style={{ ...button, marginLeft: 8 }}>Restore last valid</button></div>}
      ...
      <div className="cad-authoring-layout" style={{ display: 'grid', gridTemplateColumns: 'minmax(220px, .72fr) minmax(360px, 1.45fr) minmax(280px, .95fr)', gap: 10, padding: 10, alignItems: 'start' }}>
        ...
        <aside className="cad-authoring-column" style={{ display: 'grid', gap: 9, minWidth: 0 }}>
          ...
          <section aria-labelledby="feature-builder-title" style={{ ...card, padding: 10, display: 'grid', gap: 7 }}>
            ...
            <div className="cad-mobile-stack" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 5 }}><button type="button" disabled={!state.selectedId} onClick={() => state.selectedId && setFeatureInputs(state.selectedId)} style={button}>Use selected as input</button><button type="button" disabled={!state.selectedId} onClick={() => state.selectedId && setFeatureTargets(state.selectedId)} style={button}>Use selected as target</button></div>
            <label style={{ fontSize: 12 }}>Distance / angle / radius<input aria-label="Feature numeric value" type="number" value={featureValue} onChange={(event) => setFeatureValue(Number(event.target.value))} style={{ ...field, marginTop: 3 }} /></label>
          ...
          <section aria-labelledby="parameters-title" style={{ ...card, padding: 10, display: 'grid', gap: 7 }}>
            ...
            {state.document.parameters.map((parameter) => <div key={parameter.id} style={{ ...mono, fontSize: 12 }}>{parameter.name} = {parameter.expression}</div>)}
            <div className="cad-mobile-stack" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 5 }}>
          ...
          <section aria-labelledby="assembly-title" style={{ ...card, padding: 10, display: 'grid', gap: 7 }}>
            ...
            <div style={{ borderTop: '1px solid var(--line)', paddingTop: 7, display: 'grid', gap: 5 }}>
...
export function cadProjectTreeSections(document: CadDocument) {
  return [
    { label: 'Sketches', count: document.sketches.length, rows: document.sketches.flatMap((item) => [{ id: item.id, label: item.name, meta: `${item.entities.length} entities · ${item.constraints.length} constraints recorded, not solved · ${item.dimensions.length} dimensions recorded, not driving` }, ...item.entities.map((entity, index) => ({ id: entity.id, label: `${item.name} / ${index + 1}. ${entity.kind}`, meta: 'selectable feature input' }))]) },
    { label: 'Bodies', count: document.bodies.length, rows: document.bodies.map((item) => ({ id: item.id, label: item.name, meta: `${item.featureIds.length} features · ${item.state}` })) },
    { label: 'Instances', count: document.assembly.instances.length, rows: document.assembly.instances.map((item) => ({ id: item.id, label: item.name, meta: item.grounded ? 'grounded' : 'free' })) },
    { label: 'Mates', count: document.assembly.mates.length, rows: document.assembly.mates.map((item) => ({ id: item.id, label: item.name, meta: `${item.kind} · recorded, not solved` })) },
  ];
}

function ProjectTree({ document, selectedId, onSelect }: { document: CadDocument; selectedId: string | null; onSelect: (id: string) => void }) {
  const sections = cadProjectTreeSections(document);
  return <section aria-labelledby="project-tree-title" style={{ ...card, padding: 10 }}><h3 id="project-tree-title" style={{ margin: '0 0 8px', fontSize: 13 }}>Model tree</h3>{sections.map((section) => <div key={section.label} style={{ marginTop: 8 }}><div style={{ ...mono, fontSize: 12, fontWeight: 850, textTransform: 'uppercase', color: 'var(--muted)' }}>{section.label} · {section.count}</div>{section.count === 0 && <div style={{ fontSize: 12, color: 'var(--muted)', padding: '4px 0' }}>None authored</div>}{section.rows.map((row) => <button key={row.id} type="button" onClick={() => onSelect(row.id)} style={{ ...button, width: '100%', textAlign: 'left', marginTop: 4, background: selectedId === row.id ? 'var(--surface2)' : 'var(--surface)' }}><span>{row.label}</span><span style={{ display: 'block', ...mono, marginTop: 2, fontSize: 12, color: 'var(--muted)' }}>{row.meta}</span></button>)}</div>)}</section>;
}
...
function SketchEditor({ sketch, busy, onChange, onCommit }: { sketch: CadSketch; busy: boolean; onChange: (sketch: CadSketch) => void; onCommit: () => void }) {
  ...
  return <section aria-labelledby="sketch-editor-title" style={{ ...card, padding: 10, display: 'grid', gap: 7 }}>
    ...
    <div className="cad-mobile-stack" style={{ borderTop: '1px solid var(--line)', paddingTop: 6, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 5 }}>
    ...
    <div style={{ ...mono, fontSize: 12, color: 'var(--muted)' }}>{sketch.dimensions.length} dimensions · {sketch.constraints.length} constraints · connected OCCT may solve; browser fallback records but does not solve</div>
...
function EntityRow({ entity, onChange, onRemove, index }: { entity: SketchEntity; onChange: (entity: SketchEntity) => void; onRemove: () => void; index: number }) {
  ...
  return <div className="cad-entity-row" style={{ display: 'grid', gridTemplateColumns: '62px 1fr auto', gap: 4, alignItems: 'center', background: 'var(--surface2)', padding: 5, borderRadius: 6 }}><span style={{ ...mono, fontSize: 12 }}>{index + 1}. {entity.kind}</span><input aria-label={`${entity.kind} geometry values`} value={values.join(', ')} onChange={(event) => onChange(updateEntityNumbers(entity, event.target.value.split(',').map(Number)))} style={{ ...field, padding: '5px 6px', fontSize: 12 }} /><button type="button" aria-label={`Remove ${entity.kind}`} onClick={onRemove} style={{ ...button, padding: '4px 6px' }}>x</button></div>;
}
...
function SemanticMesh({ mesh, document }: { mesh: ReturnType<typeof createCadAuthoringState>['lastValidMesh']; document: CadDocument }) {
  ...
  return <section aria-labelledby="mesh-title" style={{ ...card, overflow: 'hidden' }}><div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 10px', borderBottom: '1px solid var(--line)' }}><h3 id="mesh-title" style={{ margin: 0, fontSize: 13 }}>Last valid semantic mesh</h3><span style={{ ...mono, fontSize: 12 }}>{mesh ? `${mesh.triangles.length} triangles` : 'no successful kernel mesh'}</span></div><svg role="img" aria-labelledby="semantic-mesh-title semantic-mesh-desc" viewBox="0 0 240 210" style={{ display: 'block', width: '100%', minHeight: 270, background: 'radial-gradient(circle at 50% 44%, var(--surface), var(--line))' }}><title id="semantic-mesh-title">Revision-bound CAD mesh projection</title><desc id="semantic-mesh-desc">Accessible two-dimensional projection of {document.bodies.length} bodies from revision {document.revisionId}.</desc><path d="M0 175 H240 M25 0 V210" stroke="var(--line)" strokeWidth=".5" />{mesh?.triangles.map((triangle, index) => { const points = triangle.map((vertex) => projected[vertex]).filter(Boolean).map((point) => point.join(',')).join(' '); return <polygon key={`${triangle.join('-')}:${index}`} points={points} fill={mesh.groups.find((group) => index >= group.startTriangle && index < group.startTriangle + group.triangleCount)?.color ?? 'var(--accent)'} fillOpacity=".52" stroke="var(--accent)" strokeWidth=".65" />; })}{!mesh && <text x="120" y="92" textAnchor="middle" fill="var(--muted)" fontSize="12">No validated mesh yet</text>}{!mesh && <text x="120" y="108" textAnchor="middle" fill="var(--muted)" fontSize="12">Queue a supported sketch and feature</text>}</svg><div style={{ padding: 8, fontSize: 12, color: 'var(--muted)' }}>This accessible SVG is a projection of the last successful connected-OCCT or bounded-browser mesh. It is not a B-rep claim.</div></section>;
}
...
function DependencyRail({ graph, history, diagnostics }: { graph: ReturnType<typeof createCadAuthoringState>['dependencyGraph']; history: ReturnType<typeof createCadAuthoringState>['history']; diagnostics: ReturnType<typeof createCadAuthoringState>['diagnostics'] }) {
  return <section aria-labelledby="dependency-title" style={{ ...card, padding: 10 }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}><h3 id="dependency-title" style={{ margin: 0, fontSize: 13 }}>Dependency & revision rail</h3><span style={{ ...mono, fontSize: 12 }}>{graph.nodes.length} nodes · {graph.edges.length} edges</span></div><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 6, marginTop: 8 }}>{graph.nodes.map((node) => <div key={node.id} style={{ padding: 7, border: '1px solid var(--line)', borderRadius: 6, fontSize: 12 }}><b>{node.label}</b><span style={{ ...mono, display: 'block', marginTop: 3, fontSize: 12 }}>{node.kind} · {node.state}</span></div>)}{graph.nodes.length === 0 && <div style={{ fontSize: 12, color: 'var(--muted)' }}>Dependency graph arrives only with an authoritative recompute or import response.</div>}</div><details style={{ marginTop: 9 }} open><summary style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', minHeight: 32, fontSize: 12, fontWeight: 800 }}>Operation history ({history.length})</summary><ol style={{ margin: '6px 0 0', paddingLeft: 20, fontSize: 12 }}>{history.slice().reverse().map((item) => <li key={item.id} style={{ padding: '3px 0' }}><b>{item.status}</b> · {item.label} · <span style={mono}>{shortId(item.revisionId)}</span></li>)}</ol></details>{diagnostics.length > 0 && <details style={{ marginTop: 8 }}><summary style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', minHeight: 32, fontSize: 12, fontWeight: 800 }}>Kernel diagnostics ({diagnostics.length})</summary>{diagnostics.map((item) => <div key={item.id} style={{ marginTop: 5, padding: 6, background: item.severity === 'error' ? 'color-mix(in srgb, var(--red) 10%, var(--surface))' : 'color-mix(in srgb, var(--amber) 12%, var(--surface))', fontSize: 12 }}><b>{item.code}</b> · {item.message}</div>)}</details>}</section>;
}
...
function TransferPanel({ message, onImport, onExport }: { message: string | null; onImport: (file: File, format: CadTransferFormat) => void; onExport: (format: CadTransferFormat) => void }) {
  return <section aria-labelledby="transfer-title" style={{ ...card, padding: 10, display: 'grid', gap: 7 }}><h3 id="transfer-title" style={{ margin: 0, fontSize: 13 }}>Kernel exchange</h3><label className="cad-file-target" style={{ ...button, textAlign: 'center' }}>Import STEP / IGES / STL<input aria-label="Import CAD file" type="file" accept=".step,.stp,.iges,.igs,.stl" style={{ display: 'none' }} onChange={(event) => { const file = event.target.files?.[0]; if (file) onImport(file, formatFromName(file.name)); }} /></label><div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 4 }}>{(['STEP', 'IGES', 'STL'] as CadTransferFormat[]).map((format) => <button key={format} type="button" onClick={() => onExport(format)} style={button}>{format}{format !== 'STL' ? ' · OCCT' : ''}</button>)}</div><p style={{ margin: 0, fontSize: 12, lineHeight: 1.4, color: 'var(--muted)' }}>Browser fallback imports and exports real STL mesh bytes. STEP/IGES require connected, owner-approved OCCT and fail closed when it is unavailable. No exchange format preserves editable feature history.</p>{message && <div role="status" style={{ fontSize: 12, padding: 6, background: 'var(--surface2)' }}>{message}</div>}</section>;
}
...
export function OutputPanel({ busy, nativeEnvelope, sealedSnapshotArtifact, bundle, message, error, retainedFormats, onNativeSeal, onNativeLoad, onGenerate, onDownload }: {
  ...
  return <section aria-labelledby="outputs-title" style={{ ...card, padding: 10, display: 'grid', gap: 7 }}>
    <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 6 }}><h3 id="outputs-title" style={{ margin: 0, fontSize: 13 }}>CADdyDaddy snapshot (.caddy.json) & manufacturing outputs</h3><span style={{ ...mono, fontSize: 12 }}>{bundle ? 'last valid' : 'not generated'}</span></div>
    <div className="cad-mobile-stack" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 5 }}>
      <button type="button" disabled={busy} onClick={onNativeSeal} style={button}>Seal current snapshot</button>
      <label className="cad-file-target" style={{ ...button, textAlign: 'center', opacity: busy ? .55 : 1 }}>Load CADdyDaddy snapshot<input aria-label="Load CADdyDaddy snapshot (.caddy.json)" type="file" accept=".json,.caddy.json,application/json" disabled={busy} style={{ display: 'none' }} onChange={(event) => { const file = event.target.files?.[0]; if (file) onNativeLoad(file); event.currentTarget.value = ''; }} /></label>
    </div>
    ...
    <div style={{ ...mono, fontSize: 12, color: 'var(--muted)' }}>Retained STEP / IGES / STL exchange · {retainedFormats.length ? retainedFormats.join(' / ') : 'none'} · CADdyDaddy snapshot {nativeEnvelope ? shortId(nativeEnvelope.document.document_hash) : 'not sealed'}</div>
    {error && <div role="alert" style={{ padding: 7, border: '1px solid var(--red)', background: 'color-mix(in srgb, var(--red) 10%, var(--surface))', color: 'var(--red)', fontSize: 12 }}>{error}</div>}
    {message && <div role="status" aria-live="polite" style={{ padding: 7, background: 'var(--surface2)', fontSize: 12, lineHeight: 1.35 }}>{message}</div>}
    {bundle && <div style={{ display: 'grid', gap: 4 }}>
      <div style={{ ...mono, fontSize: 12, overflowWrap: 'anywhere' }}>package · {bundle.package.package_id}<br />manifest · {bundle.package.manifest_file_sha256}</div>
      <div style={{ display: 'grid', gap: 3 }}>{bundle.artifacts.map((artifact) => <button className="cad-output-download" key={artifact.path} type="button" disabled={busy} onClick={() => onDownload(artifact)} aria-label={`Download ${artifact.path}`} style={{ ...button, display: 'flex', justifyContent: 'space-between', gap: 6, textAlign: 'left', opacity: busy ? .55 : 1 }}><span>Download · {artifact.path}</span><span style={{ ...mono, fontSize: 12 }}>{artifact.size_bytes} B · {shortId(artifact.sha256)}</span></button>)}</div>
    </div>}
    <details><summary style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', minHeight: 32, fontSize: 12, fontWeight: 800 }}>Output boundaries</summary><ul style={{ margin: '5px 0 0', paddingLeft: 17, fontSize: 12, lineHeight: 1.45 }}>{CAD_OUTPUT_LIMITATIONS.map((item) => <li key={item}>{item.replaceAll('_', ' ').toLowerCase()}</li>)}</ul></details>
  </section>;
}
