...
import { DESIGN_HASH_FULL, DESIGN_HASH_SHORT, ECFR_DATE } from '../lib/catalog';
  ...
  return (
    <div className="h-12 flex-none flex items-center gap-4 pl-4 pr-3 border-b border-line2 bg-surface">
      <button onClick={() => patch({ sel: null, timelineOpen: false, helpOpen: false })} title="Return to baseline" className="flex items-center gap-[10px] bg-transparent border-0 p-0 text-ink cursor-pointer min-h-6">
        ...
        <span className="font-bold tracking-[.01em]">CADdy Daddy</span>
      </button>
      <span className="text-muted text-[13px]">
        Kestrel · design hash <span className="font-mono">{DESIGN_HASH_SHORT}</span>{' '}
        <button onClick={() => copy('hash', DESIGN_HASH_FULL)} className="btn btn-xs ml-1">{copied === 'hash' ? 'copied' : 'copy'}</button>
      </span>
