...
export function IntakeForm({ value, onChange, disabled = false }: { value: Intake; onChange: (next: Intake) => void; disabled?: boolean }) {
  ...
  return (
    <div className="grid gap-4 text-[13px]">
      <fieldset className="grid grid-cols-1 sm:grid-cols-2 gap-3 border-0 p-0 m-0 min-w-0"><legend className="text-[12px] font-mono uppercase tracking-[.06em] text-muted pb-2">use case</legend>
        ...
        <label className="grid gap-1 text-muted">is the pod used on an aircraft?<select value={value.usedOn} disabled={disabled} onChange={(e) => set('usedOn', e.target.value as Intake['usedOn'])} className="field text-ink">{USED_ON.map((x) => <option key={x} value={x}>{x}</option>)}</select></label>
      </fieldset>
      ...
      <div className="flex gap-6 flex-wrap">
        <label className="flex items-center gap-3 min-h-11 cursor-pointer"><input type="checkbox" checked={value.civilProduct} disabled={disabled} onChange={(e) => set('civilProduct', e.target.checked)} /> declared a civil product <span className="chip chip-sm">declared</span></label>
        <label className="flex items-center gap-3 min-h-11 cursor-pointer"><input type="checkbox" checked={value.bvlos} disabled={disabled} onChange={(e) => set('bvlos', e.target.checked)} /> beyond visual line of sight</label>
      </div>
