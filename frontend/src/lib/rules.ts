...
export function outcome(d: Design, packId: PackId = 'v2'): Outcome {
  ...
  if (!wh || endurance == null || range == null) {
    ...
    else if (pack.enduranceAtH != null && endurance >= pack.enduranceAtH) add({ id: 'r1at', node: 'airframe', entry: '9A012.a.1', kind: 'CCL', reason: 'AT only · endurance crossed 30 min · pack v1', number: 'endurance ' + f2(endurance) + ' h ≥ 0.5 h', sentence: 'a.1. An endurance of 30 minutes or greater but less than 1 hour;', ecfr: E, eff: pack.effective, fr: 'pre-2026-08-13 text', url: 'ecfr.gov/…/part-774/supplement-1 · 9A012', atoms: ['AT column · NLR to every destination modelled'], cols: 'NLR' });
